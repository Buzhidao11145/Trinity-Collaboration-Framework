...
**我想了解……**
- 🏗️ [Trinity是什么？](#为什么需要-trinity) — 框架的核心问题和解决方案
- 👥 [三个角色如何分工？](#三层角色分离) — 决策者、规划者、执行者的职责与边界
- 🛡️ [什么是缓冲层？](#缓冲层机制核心创新) — Trinity的核心创新设计
- 🔄 [日常如何协作？](#六步闭环协作流程) — 从想法到复盘的完整流程
- 📚 [有什么理论支撑？](#理论基石) — 五大理论框架
- 🗂️ [如何管理知识？](#信息架构外部认知系统) — 知识仓库、简报、索引体系
- 🔧 [如何开始使用？](#快速开始) — 三步搭建你的Trinity系统
- 🌍 [有英文版吗？](#多语言) — 见 `README_EN.md`

**我是……**
- 🧑‍💻 **AI工具使用者** → 直接看 [快速开始](#快速开始)
- 🔬 **安全研究者** → 重点看 [实战案例](#实战验证) + [缓冲层机制](#缓冲层机制核心创新)
- 🏗️ **系统设计者** → 重点看 [理论基石](#理论基石) + [设计文档](docs/)
- 🤝 **开源贡献者** → 重点看 [贡献指南](#贡献指南) + [决策知识库](decisions/决策知识库.md)
...
- [认知共同体：三个角色，一个系统](#认知共同体三个角色一个系统)
  - [行为与认知的双向塑造](#行为与认知的双向塑造)
- [缓冲层机制：核心创新](#缓冲层机制核心创新)
- [自适应规划：感知决策者的状态](#自适应规划感知决策者的状态)
  - [情绪变量：让系统感知人的状态](#情绪变量让系统感知人的状态)
- [六步闭环协作流程](#六步闭环协作流程)
...
- [与 AI Agent 的直接对比](#与-ai-agent-的直接对比)
  - [本质区别：决策权的分配](#本质区别决策权的分配)
- [局限性](#局限性)
  - [理论容错性与工程容错性](#理论容错性与工程容错性)
- [快速开始](#快速开始)
- [适用场景](#适用场景)
- [贡献指南](#贡献指南)
- [许可证](#许可证)
...
在 Trinity 中，塑造关系是**双向**的——不仅仅是规划者在"教"执行者怎么做事，执行者的行为也在"教"规划者如何更好地规划。
...
- **LangGraph 和 CrewAI** 聚焦于 Agent 之间的**任务分配**——谁做什么、什么顺序做。它们是在 AI 之间分配工作。
- **AutoGPT 和 BabyAGI** 强调 AI **自主性**——AI 自己设定目标、自己执行。它们是在减少人的参与。
- **Trinity** 聚焦于**决策权的分配**——什么决策由人做、什么决策由 AI 做、什么决策需要人确认后才能执行。它是在定义人与 AI 之间的权力边界。
...
Trinity 在**工程层面**和**理论层面**都有完整的容错设计，但它们的性质不同。

**工程容错性**是指系统在角色缺失、信息丢失、决策偏差时如何兜底和恢复：

- 角色缺失预案：规划者宕机了怎么办？执行者越界了怎么办？（见 [反脆弱：角色缺失预案](#角色缺失预案)）
- 信息丢失兜底：知识仓库损坏了怎么重建？每条信息有几个备份？（见 [反脆弱：信息丢失兜底](#信息丢失兜底)）
- 决策偏差纠正：发现错误决策后怎么回滚？（见 [反脆弱：决策偏差纠正](#决策偏差纠正)）

工程容错性做了，理论和实证都做了——有自检清单、脉搏日志、信息保真机制、知识仓库备份策略。这部分设计方案在 [docs/反脆弱设计.md](docs/反脆弱设计.md) 中有完整记录。

**理论容错性**是指框架的思想本身如何接受挑战和修正。每个核心假设都标注了置信度，并在设计上预留了修正空间：
...
1. **每个结论标注置信度**（🟢 高 / 🟡 推测 / 🔴 不确定），区分"已验证"和"推测"。
2. **每个决策规则标注来源**，可追溯到具体实践。没有来源的规则被视为"暂定"。
3. **核心原则保留修订历史**，不是删除旧版本而是标注"已被新规则替代"——让思想演化过程可追溯。
4. **每月一次框架复审**，检查假设是否仍然成立。如果某个核心假设被证伪，不是掩盖它，而是把它结构化为学习记录。
...
*Trinity AI 协作框架 v1.2 · 2026 年 6 月*
*"在能力和行动之间，留一个思考的间隙。"*
